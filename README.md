...
A blockchained relational database (BRDB) is an immutable relational database. A BRDB framework is a foundation to make any relational database immutable by hashing all activities on the database into a proof-of-work (PoW) blockchain. Each activity is recorded as a data on its own. A hash is done on the activity data. A PoW is an expensive hashing technique which makes anything that it is representing immutable. It is also expensive to rehash a data where any changes to the data always requires rehashing. A hash represents the data it hashes. The data may be another hash, and so on. Hence, a hash may represent a complex hashing structure of a large data set. A blockchain is a list of inclusive PoW. An inclusive hash is a hash that includes an earlier hash which in turn includes the hash before it, and so on. Hence, an inclusive PoW is very immutable. Hence, anything a PoW blockchain represents is very immutable. All these hashing will be known as the BRDB methodology.
